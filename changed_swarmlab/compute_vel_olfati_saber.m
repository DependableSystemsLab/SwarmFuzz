function [vel_command, collisions] = compute_vel_olfati_saber(self, p_swarm, r_agent, dt ,time, start_t, dur, att_id, vic_id1, vic_id2, dev_y, dire_csv, posY_csv, dist_csv)
    ...
    nb_vic_collisions = 0; % Nb of collisions among two victim drones, added by Elaine
    flag_list= ones(1,nb_agents);
    ...
    for agent = 1:nb_agents
        ...
        p_rel = pos - pos(:, agent);
        dist = sqrt(sum((p_rel.^2), 1)); % How to find the dist with vic_id2?
        D(agent, :) = dist; % Is D(x, :)1x5?
        % ------ 4.1 dist_csv: record the distance between two victim drones 
        if (agent == vic_id1)
            % Add the direction for the distance
            % We only record the inter-dist when vic 1 is close to the
            % obstacle
            if abs(pos(1, vic_id1)-93.5)<10
                if pos(2,vic_id1)> pos(2,vic_id2)
                    dist12 = D(agent, vic_id2);
                else
                    dist12 = -1*(D(agent, vic_id2));
                end
                res_dist_mat = [time, dist12];
                writematrix(res_dist_mat, dist_csv, 'Delimiter', ',', 'WriteMode', 'append');
            end
        end
% ------ 4.2 dire_csv: record the coord of the swarm member passing the obstacle
        if (flag_list(agent)&&(pos(1,agent)> 100)&& (pos(1,agent)<101))
            loc_obs_mat = [agent, pos(1:2, agent)'];
            flag_list(agent) = 0;
            writematrix(loc_obs_mat, dire_csv, 'Delimiter', ',', 'WriteMode', 'append');
        end
    ...
    % --------- 1. posY_csv: record each drone's y location at each timestamp 
    pos_mat = zeros(1, p_swarm.nb_agents);
    for i = 1:p_swarm.nb_agents
        pos_mat(i) = pos(1,i);
    end
    writematrix(pos_mat, posY_csv, 'Delimiter', ',', 'WriteMode', 'append');
   
    % ------ 2. col_csv: record the nb of collisions betweent the victim drone and the obstacle 
